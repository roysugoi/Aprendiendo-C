...
int main() 				//Esta es la funci�n principal (main) que vamos a crear. "Int" quiere decir que es un número entero.		
{
	...
	float hipotenusa,cateto1,cateto2;						//Dado que es muy probable que salga un número REAL, utilizaremos la expresi�n float en vez de int para este ejercicio.
	...
	hipotenusa = sqrt(pow(cateto1,2)+pow(cateto2,2));		//Utilizamos la f�rmula de Pit�goras, la cual podemos utilizar gracias a la librer�a math.h. sqrt equivale a ra�z, y pow equivale a potencia. A continuaci�n de pow, indicamos el número base, y tras la coma, la potencia.
