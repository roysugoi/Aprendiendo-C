...
int main(){				//Comenzamos con int main(){}, lo cual quiere decir que trabajaremos con una funci�n PRINCIPAL que maneja números enteros (int).
	...
	char a = 'e';				//"Char" viene de character, y puede ser un elemento cualquiera, sea número, letra o s�mbolo. Aqu� decimos que el caracter "a" pasa a ser "e". BYTES: 1, RANGO: 0 .. 255.
	...
	printf("El char es %c \n",a);			//El % indica qu� tipo de dato es, al ser un char (pudiendo no ser un número), se escribe %c. A continuaci�n, decimos que imprima el valor "a". \n sirve para crear una l�nea entre este y el siguiente rengl�n.

	printf("El short es %i \n",b);			//El % indica qu� tipo de dato es, al ser un short de un número entero, se escribe %i. A continuaci�n, decimos que imprima el valor "b".\n sirve para crear una l�nea entre este y el siguiente rengl�n.
	...
	printf("El float es %.2f \n",f);		//Al referirnos a un número real con decimales usamos %f. El ".2" lo hemos a�adido porque no queremos much�simos decimales, con dos nos vale. Los decimales se redondean.
